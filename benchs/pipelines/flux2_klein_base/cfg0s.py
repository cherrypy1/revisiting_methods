...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import Flux2KleinCFG0SPipeline

        >>> pipe = Flux2KleinCFG0SPipeline.from_pretrained(
        ...
        >>> image = pipe(prompt, num_inference_steps=50, guidance_scale=4.0, zero_steps=1).images[0]
        >>> image.save("flux2_cfg0s_output.png")
        ```
"""
...
def optimized_scale(
    ...
    positive_flat = positive_flat.to(dtype=torch.float32)
    negative_flat = negative_flat.to(dtype=torch.float32)
...
class Flux2KleinCFG0SPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    r"""
    The Flux2 Klein pipeline with CFG-Zero* for text-to-image generation.
    ...
    def __call__(
        ...
        Args:
            ...
            use_cfg_zero_star (`bool`, *optional*, defaults to `True`):
                Enables CFG-Zero* optimized scaling of the unconditional velocity branch.
            use_zero_init (`bool`, *optional*, defaults to `True`):
                Enables zeroing out the first `zero_steps` ODE solver steps.
            zero_steps (`int`, *optional*, defaults to `1`):
                Number of initial denoising steps to zero out. `1` matches the paper default.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    if use_zero_init and i < zero_steps:
                        noise_pred = torch.zeros_like(noise_pred)
                    elif use_cfg_zero_star:
                        positive_flat = noise_pred.reshape(noise_pred.shape[0], -1)
                        negative_flat = neg_noise_pred.reshape(
                            neg_noise_pred.shape[0], -1
                        )
                        alpha = optimized_scale(positive_flat, negative_flat)
                        alpha = alpha.view(
                            noise_pred.shape[0], *([1] * (noise_pred.ndim - 1))
                        ).to(dtype=noise_pred.dtype)

                        neg_noise_pred = neg_noise_pred * alpha
