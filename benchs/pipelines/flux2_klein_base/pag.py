...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import Flux2KleinPAGPipeline

        >>> pipe = Flux2KleinPAGPipeline.from_pretrained(
        ...
        >>> image = pipe(prompt, num_inference_steps=50, guidance_scale=4.0, pag_scale=1.5).images[0]
        >>> image.save("flux2_pag_output.png")
        ```
"""


def _expand_pag_applied_layers(pag_applied_layers: list[str] | None) -> set[str] | None:
    if pag_applied_layers is None:
        return None

    expanded_layers: set[str] = set()
    for layer_name in pag_applied_layers:
...
def _make_pag_identity_mask(
    ...
    if start_index < 0 or end_index > seq_len:
        return attention_mask

    pag_mask = torch.zeros((seq_len, seq_len), device=query.device, dtype=query.dtype)
    pag_mask[start_index:end_index, start_index:end_index] = float("-inf")
    index = torch.arange(start_index, end_index, device=query.device)
    pag_mask[index, index] = 0

    return pag_mask if attention_mask is None else attention_mask + pag_mask


class Flux2PAGAttnProcessor(Flux2AttnProcessor):
    def __init__(self):
        super().__init__()
    ...
    def __call__(
        ...
        pag_image_seq_len: int | None = None,
        pag_text_seq_len: int | None = None,
    ) -> torch.Tensor:
        ...
        pag_start_index = encoder_hidden_states_seq_len
        if kv_cache_mode == "extract" and num_ref_tokens > 0:
            pag_start_index = encoder_hidden_states_seq_len + num_ref_tokens

        if self.do_perturb:
            attention_mask = _make_pag_identity_mask(
                query,
                pag_start_index,
                pag_image_seq_len,
                attention_mask,
        ...
        attention_backend = "native" if self.do_perturb else self._attention_backend
...
class Flux2PAGParallelSelfAttnProcessor(Flux2ParallelSelfAttnProcessor):
    def __init__(self):
        super().__init__()
        self.do_perturb = False
    ...
    def __call__(
        ...
        batch_size = hidden_states.shape[0]
        text_seq_len = (
            pag_text_seq_len if pag_text_seq_len is not None else num_txt_tokens
        )
        ...
        pag_start_index = text_seq_len
        if kv_cache_mode == "extract" and num_ref_tokens > 0:
            pag_start_index = text_seq_len + num_ref_tokens
...
class Flux2KleinPAGPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    r"""
    The Flux2 Klein pipeline with Perturbed-Attention Guidance for text-to-image generation.
    ...
    def __call__(
        ...
        pag_scale: float = 1.5,
        pag_applied_layers: list[str] | None = None,
        num_images_per_prompt: int = 1,
        ...
        Args:
            ...
            pag_scale (`float`, *optional*, defaults to 1.5):
                PAG scale for the identity-attention correction.
            pag_applied_layers (`list[str]`, *optional*):
                Layer tags where PAG perturbation is injected. Supports explicit tags (e.g. `d3`, `s7`) and ranges
        ...
        if pag_applied_layers is None:
            pag_applied_layers = ["d2"]
        self._pag_applied_layers = _expand_pag_applied_layers(pag_applied_layers)
        ...
        if self.do_pag:
            pag_attention_kwargs = {
                "pag_image_seq_len": latents.shape[1],
                "pag_text_seq_len": text_ids.shape[1],
            }
            if self._attention_kwargs is None:
                self._attention_kwargs = pag_attention_kwargs
            else:
                self._attention_kwargs = self._attention_kwargs.copy()
                self._attention_kwargs.update(pag_attention_kwargs)
        ...
        if self.do_pag:
            ...
            apply_all_layers = self.pag_applied_layers is None
            ...
            for name, processor in list(procs.items()):
                ...
                if parts[0] == "transformer_blocks":
                    ...
                    use_for_pag = apply_all_layers or layer_tag in self.pag_applied_layers
                    replace_processor = Flux2PAGAttnProcessor()
                elif parts[0] == "single_transformer_blocks":
                    layer_tag = f"s{block_num}"
                    use_for_pag = apply_all_layers or layer_tag in self.pag_applied_layers
                    replace_processor = Flux2PAGParallelSelfAttnProcessor()
                ...
                if replace_processor is not None:
                    ...
                    if use_for_pag:
                        pag_processors.append(replace_processor)
            ...
            if len(pag_processors) == 0:
                raise ValueError(
                    "PAG is enabled, but no attention layers matched "
                    f"pag_applied_layers={pag_applied_layers}. "
        ...
        try:
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    if self.do_pag:
                        for processor in pag_processors:
                    ...
                    if self.do_pag:
                        ...
                        try:
                            with self.transformer.cache_context("pag"):
                    ...
                    if self.do_classifier_free_guidance and self.do_pag:
                        noise_pred = (
                            ...
                            + self.pag_scale * (noise_pred_original - noise_pred_perturb)
                        )
                        ...
                        noise_pred = noise_pred_original + self.pag_scale * (
