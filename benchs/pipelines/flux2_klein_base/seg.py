...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import Flux2KleinSegPipeline

        >>> pipe = Flux2KleinSegPipeline.from_pretrained(
...
class Flux2SEGAttnProcessor(Flux2AttnProcessor):
...
class Flux2SEGParallelSelfAttnProcessor(Flux2ParallelSelfAttnProcessor):
    def __init__(self, blur_sigma=1.0, inf_blur_threshold=9999.0):
        super().__init__()
        self.blur_sigma = blur_sigma
        self.inf_blur = self.blur_sigma > inf_blur_threshold
...
class Flux2KleinSegPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    r"""
    The Flux2 Klein pipeline with Smoothed Energy Guidance for text-to-image generation.
    ...
    def __call__(
        ...
        Args:
            ...
            seg_scale (`float`, *optional*, defaults to 3.0):
                SEG scale controlling the strength of the smoothed energy correction.
            seg_blur_sigma (`float`, *optional*, defaults to 9999999.0):
                Gaussian blur sigma for the SEG perturbation branch. Very large values use the spatial mean.
            seg_applied_layers (`list[str]`, *optional*):
                Layer tags where SEG perturbation is injected. Supports explicit tags (e.g. `d3`, `s7`) and ranges
                (e.g. `d0..12`, `s0..10`). If `None`, SEG is applied to all Flux2 attention processor layers.
        ...
        if self.do_seg:
            ...
            for name, processor in list(procs.items()):
                ...
                if parts[0] == "transformer_blocks":
                    ...
                    replace_processor = Flux2SEGAttnProcessor(blur_sigma=seg_blur_sigma)
                elif parts[0] == "single_transformer_blocks":
                    layer_tag = f"s{block_num}"
                    use_for_seg = (
                        apply_all_layers or layer_tag in self.seg_applied_layers
                    )
                    replace_processor = Flux2SEGParallelSelfAttnProcessor(
            ...
            if len(seg_processors) == 0:
                raise ValueError(
                    "SEG is enabled, but no attention layers matched "
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    if self.do_seg:
                        ...
                        try:
                            with self.transformer.cache_context("seg"):
                                noise_pred_perturb = self.transformer(
                                    hidden_states=latent_model_input,
                                    timestep=timestep / 1000,
                                    guidance=None,
                                    encoder_hidden_states=seg_prompt_embeds,
                                    txt_ids=seg_text_ids,
                                    img_ids=latent_image_ids,
                    ...
                    if self.do_classifier_free_guidance and self.do_seg:
                        noise_pred = (
                            neg_noise_pred
                            + guidance_scale * (noise_pred_original - neg_noise_pred)
                            + self.seg_scale * (neg_noise_pred - noise_pred_perturb)
                        )
                        ...
                        noise_pred = noise_pred_original + self.seg_scale * (
