...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import Flux2KleinAPGPipeline

        >>> pipe = Flux2KleinAPGPipeline.from_pretrained(
        ...
        >>> image.save("flux2_apg_output.png")
        ```
"""
...
def _split_parallel_orthogonal_components(
    ...
    unit_reference = reference / reference_norm.clamp_min(eps)
...
class Flux2KleinAPGPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    r"""
    The Flux2 Klein pipeline with APG for text-to-image generation.
    ...
    def __call__(
        ...
        Args:
            ...
            apg_step_radius (`float`, *optional*, defaults to 0.0):
                Fixed L2 radius used to clamp the raw APG update norm. Set `<= 0` to disable clamping.
            apg_momentum (`float`, *optional*, defaults to -0.5):
                Reverse-inertia coefficient for the APG update. Negative values apply reverse momentum.
