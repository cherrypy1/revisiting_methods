...
def config_path(method, model="sd35"):
    # Prefer new yaml-based configs/<model>/<method>.yaml; fall back to legacy
    # generation/configs/<model>/<method>.py for backwards compat.
    candidates = [
        PROJECT_ROOT / "configs" / model / f"{method}.yaml",
        PROJECT_ROOT / "generation" / "configs" / model / f"{method}.py",
    ]
    for cfg in candidates:
        if cfg.exists():
            return cfg
    sys.exit(f"Config not found: tried {[str(c) for c in candidates]}")


def run_geneval(args, cfg, out_root):
    prompts = env_path("GENEVAL_PROMPTS", PROJECT_ROOT / "prompts" / "evaluation_metadata.jsonl")
    ...
    if args.skip_eval:
        return
    models = env_path("GENEVAL_MODELS", PROJECT_ROOT / "models")
    mm_config = os.environ.get(
        "GENEVAL_MM_CONFIG",
        str(PROJECT_ROOT / "mmdetection/configs/mask2former/"
...
def run_oneig(args, cfg, out_root):
    csv = env_path("ONEIG_CSV", "/home/aaturevich/OneIG-Benchmark/OneIG-Bench.csv")
    ...
    if args.skip_eval:
        return
    oneig_root = env_path("ONEIG_ROOT", "/home/aaturevich/OneIG-Benchmark")
...
def run_dpg(args, cfg, out_root):
    prompts_dir = env_path("DPG_PROMPTS", "/home/aaturevich/ELLA/dpg_bench/prompts")
    ...
    if args.skip_eval:
        return
    dpg_root = env_path("DPG_ROOT", "/home/aaturevich/ELLA")
