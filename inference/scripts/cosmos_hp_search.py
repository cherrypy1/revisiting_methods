...
sys.path.insert(0, str(PROJECT_ROOT / "generation" / "configs" / "cosmos2"))

from scripts.common import make_generator, pick_device, write_prompt_sidecar  # noqa: E402

DEFAULT_PROMPTS = PROJECT_ROOT / "prompts" / "evaluation_metadata.jsonl"
...
def parse_args():
    ...
    p.add_argument("--method-config", default=str(PROJECT_ROOT / "generation" / "configs" / "cosmos2" / "cfg.py"),
...
def main():
    ...
    from _patch import load_pipeline as build_pipe  # noqa: E402
    ...
    for method in args.methods:
        ...
        for trial in GRIDS[method]:
            ...
            for idx, meta in enumerate(chosen):
                ...
                with torch.no_grad():
                    image = pipe(prompt=meta["prompt"], **kwargs).images[0]
                image.save(img_path)
