...
Usage:
    scripts/eval_oneig.py --oneig-root $ONEIG_ROOT \\
...
def main():
    ...
    run([sys.executable, "-m", "scripts.alignment.alignment_score",
         "--image_dirname", str(image_dir),
         "--class_items", "object",
         *common], cwd=oneig, env=env)

    # text: reads image_dirname/<model>/<id>*
    run([sys.executable, "-m", "scripts.text.text_score",
         "--image_dirname", str(image_dir / "text"),
         *common], cwd=oneig, env=env)

    # reasoning: reads image_dirname/<model>/*
    run([sys.executable, "-m", "scripts.reasoning.reasoning_score",
