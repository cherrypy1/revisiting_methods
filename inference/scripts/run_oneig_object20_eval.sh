#!/usr/bin/env bash
# Fast OneIG object-only validation over already generated images.
#
...
PROMPTS=${ONEIG_OBJECT_PROMPTS:-20}
PARTITION=${ONEIG_OBJECT_PARTITION:-rocky}
GPU_TYPE=${ONEIG_OBJECT_GPU_TYPE:-v100}
CPUS=${ONEIG_OBJECT_CPUS:-6}
WALLTIME=${ONEIG_OBJECT_WALLTIME:-02:00:00}
PYTHON_BIN=${ONEIG_OBJECT_PYTHON:-$HOME/_.venv/.venv/bin/python}
if [[ "$PROMPTS" == "all" || "$PROMPTS" == "ALL" || "$PROMPTS" == "full" || "$PROMPTS" == "FULL" ]]; then
    PROMPT_SUFFIX=object_all
else
    PROMPT_SUFFIX=object${PROMPTS}
fi
SHARD_DIR=${ONEIG_OBJECT_SHARD_DIR:-outputs/_manual_shards/${PROMPT_SUFFIX}}
HF_HOME=${HF_HOME:-$HOME/.cache/huggingface}

echo "== OneIG object-only eval =="
echo "root:       $ROOT"
...
echo
echo "== Remove OneIG eval scratch only =="
find outputs -maxdepth 2 -type d -name '_evalwork' -path '*_oneig/*' -print
find outputs -maxdepth 2 -type d -name '_evalwork' -path '*_oneig/*' -exec rm -rf {} +

echo
echo "== Build manual object shards =="
mkdir -p "$SHARD_DIR"
...
suffix = "object_all" if limit is None else f"object{limit}"
...
for campaign in sorted(base.glob("*_oneig")):
    ...
    if limit is not None:
        ...
        if len(item_ids) < limit:
            print(f"SKIP {campaign.name}: only {len(item_ids)} generated object items")
            continue
    elif not item_ids:
        print(f"SKIP {campaign.name}: no generated object items")
        continue
...
if written == 0:
    raise SystemExit("no object eval shards were written")
PY

echo
echo "== Submit object eval workers =="
...
cd "$ROOT"
export TMPDIR=/tmp/\$USER/oneig_obj${PROMPTS}_\$SLURM_JOB_ID
mkdir -p "\$TMPDIR"
...
echo "  for d in outputs/*_oneig; do echo \"### \$d\"; cat \"\$d/flux2_klein_base\"/*/oneig/object/_summary.json 2>/dev/null || true; done"
