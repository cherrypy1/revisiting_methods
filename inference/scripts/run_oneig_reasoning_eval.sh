#!/usr/bin/env bash
# Fast OneIG reasoning-only validation over already generated images.
#
...
PROMPTS=${ONEIG_REASONING_PROMPTS:-${ONEIG_REASON_PROMPTS:-all}}
PARTITION=${ONEIG_REASONING_PARTITION:-${ONEIG_REASON_PARTITION:-rocky}}
GPU_TYPE=${ONEIG_REASONING_GPU_TYPE:-${ONEIG_REASON_GPU_TYPE:-v100}}
CPUS=${ONEIG_REASONING_CPUS:-${ONEIG_REASON_CPUS:-6}}
WALLTIME=${ONEIG_REASONING_WALLTIME:-${ONEIG_REASON_WALLTIME:-08:00:00}}
PYTHON_BIN=${ONEIG_REASONING_PYTHON:-${ONEIG_REASON_PYTHON:-$HOME/_.venv/.venv/bin/python}}
if [[ "$PROMPTS" == "all" || "$PROMPTS" == "ALL" || "$PROMPTS" == "full" || "$PROMPTS" == "FULL" ]]; then
    PROMPT_SUFFIX=reasoning_all
else
    PROMPT_SUFFIX=reasoning${PROMPTS}
fi
SHARD_DIR=${ONEIG_REASONING_SHARD_DIR:-${ONEIG_REASON_SHARD_DIR:-outputs/_manual_shards/${PROMPT_SUFFIX}}}
HF_HOME=${HF_HOME:-$HOME/.cache/huggingface}

echo "== OneIG reasoning-only eval =="
echo "root:       $ROOT"
...
echo
echo "== Remove stale OneIG reasoning eval scratch only =="
find outputs -maxdepth 3 -type d -path "*/_evalwork/*_oneig_reasoning_eval" -print
find outputs -maxdepth 3 -type d -path "*/_evalwork/*_oneig_reasoning_eval" -exec rm -rf {} +

echo
echo "== Build manual reasoning shards =="
mkdir -p "$SHARD_DIR"
...
suffix = "reasoning_all" if limit is None else f"reasoning{limit}"
...
for campaign in sorted(base.glob("*_oneig")):
    ...
    if limit is not None:
        ...
        if len(item_ids) < limit:
            print(f"SKIP {campaign.name}: only {len(item_ids)} generated reasoning items")
            continue
    elif not item_ids:
        print(f"SKIP {campaign.name}: no generated reasoning items")
        continue
...
if written == 0:
    raise SystemExit("no reasoning eval shards were written")
PY

echo
echo "== Submit reasoning eval workers =="
...
cd "$ROOT"
export TMPDIR=/tmp/\$USER/oneig_reasoning${PROMPTS}_\$SLURM_JOB_ID
mkdir -p "\$TMPDIR"
unset ONEIG_HF_HOME CFG_HF_HOME
export HF_HOME=$HF_HOME
mkdir -p "\$HF_HOME"
export HF_HUB_DISABLE_XET=1
"$PYTHON_BIN" scripts/patch_oneig_reasoning_v100.py --prepare-code
...
echo "  for d in outputs/*_oneig; do echo \"### \$d\"; cat \"\$d/flux2_klein_base\"/*/oneig/reasoning/_summary.json 2>/dev/null || true; done"
