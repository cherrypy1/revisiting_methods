...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import Flux2KleinCFGPPPipeline

        >>> pipe = Flux2KleinCFGPPPipeline.from_pretrained(
        ...
        >>> image = pipe(prompt, num_inference_steps=50, guidance_scale=1.5).images[0]
        >>> image.save("flux2_cfgpp_output.png")
        ```
"""
...
class Flux2KleinCFGPPPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    r"""
    The Flux2 Klein pipeline with CFG++ for text-to-image generation.
    ...
    def check_inputs(
        ...
        if guidance_scale >= 0.0 and self.config.is_distilled:
            logger.warning(
                f"CFG++ guidance scale {guidance_scale} is ignored for step-wise distilled models."
            )
    ...
    def do_classifier_free_guidance(self):
        return self._guidance_scale >= 0 and not self.config.is_distilled
