...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import Flux2KleinOSEGPipeline

        >>> pipe = Flux2KleinOSEGPipeline.from_pretrained(
        ...     "black-forest-labs/FLUX.2-klein-base-9B", torch_dtype=torch.float16
        ... )
        >>> pipe.to("cuda")
        >>> prompt = "A cat holding a sign that says hello world"
        >>> # Depending on the variant being used, the pipeline call will slightly vary.
        >>> # Refer to the pipeline documentation for more details.
        >>> image = pipe(prompt, num_inference_steps=50, guidance_scale=4.0, oseg_scale=2.0).images[0]
        >>> image.save("flux2_oseg_output.png")
        ```
"""
...
def _expand_seg_applied_layers(seg_applied_layers: list[str] | None) -> set[str] | None:
    if seg_applied_layers is None:
        return None

    expanded_layers: set[str] = set()
    for layer_name in seg_applied_layers:
...
def _blur_flux2_orthogonal_query(
    query: torch.Tensor,
    key: torch.Tensor,
    start_index: int,
    ...
    if image_seq_len is None or image_height is None or image_width is None:
        return query

    if image_height * image_width != image_seq_len:
        return query
    ...
    query_prefix = query[:, :start_index]
    query_image = query[:, start_index:end_index]
    ...
    query_image = query_image.permute(0, 2, 3, 1).reshape(
        batch_size, num_heads * head_dim, image_height, image_width
    )
    ...
    return torch.cat([query_prefix, query_image, query_suffix], dim=1)


class Flux2OSEGAttnProcessor(Flux2AttnProcessor):
    def __init__(self, blur_sigma=1.0, inf_blur_threshold=9999.0):
        super().__init__()
        self.blur_sigma = blur_sigma
        self.inf_blur = self.blur_sigma > inf_blur_threshold
    ...
    def __call__(
        ...
        seg_image_height: int | None = None,
        seg_image_width: int | None = None,
        seg_image_seq_len: int | None = None,
        seg_text_seq_len: int | None = None,
    ) -> torch.Tensor:
        ...
        seg_start_index = encoder_hidden_states_seq_len
        if kv_cache_mode == "extract" and num_ref_tokens > 0:
            seg_start_index = encoder_hidden_states_seq_len + num_ref_tokens

        if self.do_perturb:
            query = _blur_flux2_orthogonal_query(
                query,
                key,
                seg_start_index,
                seg_image_seq_len,
...
class Flux2OSEGParallelSelfAttnProcessor(Flux2ParallelSelfAttnProcessor):
    def __init__(self, blur_sigma=1.0, inf_blur_threshold=9999.0):
        super().__init__()
        self.blur_sigma = blur_sigma
        self.inf_blur = self.blur_sigma > inf_blur_threshold
    ...
    def __call__(
        ...
        batch_size = hidden_states.shape[0]
        text_seq_len = (
            seg_text_seq_len if seg_text_seq_len is not None else num_txt_tokens
        )
        ...
        seg_start_index = text_seq_len
        if kv_cache_mode == "extract" and num_ref_tokens > 0:
            seg_start_index = text_seq_len + num_ref_tokens
...
class Flux2KleinOSEGPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    r"""
    The Flux2 Klein pipeline with Orthogonal Smoothed Energy Guidance for text-to-image generation.
    ...
    @property
    def oseg_scale(self):
        return self._oseg_scale

    @property
    def do_oseg(self):
        return (self._seg_scale + self._oseg_scale) > 0
    ...
    def __call__(
        ...
        seg_scale: float = 0.0,
        oseg_scale: float = 2.0,
        seg_blur_sigma: float = 9999999.0,
        seg_applied_layers: list[str] | None = None,
        num_images_per_prompt: int = 1,
        ...
        Args:
            ...
            seg_scale (`float`, *optional*, defaults to 0.0):
                Additional SEG-style coefficient added to `oseg_scale` in the OSEG perturbation direction.
            oseg_scale (`float`, *optional*, defaults to 2.0):
                OSEG coefficient controlling the orthogonal smoothed energy correction.
            seg_blur_sigma (`float`, *optional*, defaults to 9999999.0):
                Gaussian blur sigma for the OSEG perturbation branch. Very large values use the spatial mean.
            seg_applied_layers (`list[str]`, *optional*):
                Layer tags where OSEG perturbation is injected. Supports explicit tags (e.g. `d3`, `s7`) and ranges
                (e.g. `d0..4`, `s0..19`). If `None`, OSEG is applied to all Flux2 attention processor layers.
        ...
        self._seg_applied_layers = _expand_seg_applied_layers(seg_applied_layers)
        ...
        if self.do_oseg:
            ...
            seg_attention_kwargs = {
                "seg_image_height": latent_height // 2,
                "seg_image_width": latent_width // 2,
                "seg_image_seq_len": latents.shape[1],
                "seg_text_seq_len": text_ids.shape[1],
            }
            if self._attention_kwargs is None:
                self._attention_kwargs = seg_attention_kwargs
            else:
                self._attention_kwargs = self._attention_kwargs.copy()
                self._attention_kwargs.update(seg_attention_kwargs)
        ...
        if self.do_oseg:
            ...
            apply_all_layers = self.seg_applied_layers is None
            seg_processor_names = []
            ...
            for name, processor in list(procs.items()):
                ...
                if parts[0] == "transformer_blocks":
                    ...
                    use_for_seg = (
                        apply_all_layers or layer_tag in self.seg_applied_layers
                    )
                    replace_processor = Flux2OSEGAttnProcessor(blur_sigma=seg_blur_sigma)
                elif parts[0] == "single_transformer_blocks":
                    layer_tag = f"s{block_num}"
                    use_for_seg = (
                        apply_all_layers or layer_tag in self.seg_applied_layers
                    )
                    replace_processor = Flux2OSEGParallelSelfAttnProcessor(
                ...
                if replace_processor is not None:
                    ...
                    if use_for_seg:
                        seg_processor_names.append(name)
            ...
            try:
                ...
                seg_processors = [
                    active_procs[name]
                    for name in seg_processor_names
            ...
            if len(seg_processors) == 0:
                raise ValueError(
                    "OSEG is enabled, but no attention layers matched "
                    f"seg_applied_layers={seg_applied_layers}. "
        ...
        try:
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    if self.do_oseg:
                        for processor in seg_processors:
                            processor.do_perturb = False
                    ...
                    neg_noise_pred = None
                    neg_noise_pred_perturb = None
                    noise_pred_perturb = None

                    if self.do_classifier_free_guidance and not self.do_oseg:
                    ...
                    if self.do_oseg:
                        for processor in seg_processors:
                            processor.do_perturb = True

                        try:
                            if self.do_classifier_free_guidance:
                                with self.transformer.cache_context("uncond_oseg"):
                                    neg_noise_pred_perturb = self.transformer(
                                        hidden_states=latent_model_input,
                                        timestep=timestep / 1000,
                                        guidance=None,
                                        encoder_hidden_states=negative_prompt_embeds,
                                ...
                                neg_noise_pred_perturb = neg_noise_pred_perturb[
                                    :, : latents.size(1) :
                                ]

                            with self.transformer.cache_context("oseg"):
                    ...
                    oseg_strength = self.seg_scale + self.oseg_scale
                    if self.do_classifier_free_guidance and self.do_oseg:
                        noise_pred = (
                            noise_pred_original
                            + (guidance_scale - 1.0)
                            * (noise_pred_original - neg_noise_pred_perturb)
                            + oseg_strength * (noise_pred_original - noise_pred_perturb)
                        )
                        ...
                        noise_pred = noise_pred_original + oseg_strength * (
