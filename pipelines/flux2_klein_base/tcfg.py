...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import Flux2KleinTCFGPipeline

        >>> pipe = Flux2KleinTCFGPipeline.from_pretrained(
        ...
        >>> image = pipe(prompt, num_inference_steps=50, guidance_scale=4.0, tcfg_rank=1).images[0]
        >>> image.save("flux2_tcfg_output.png")
        ```
"""
...
def _tcfg_project_uncond_with_svd(
    noise_pred_text: torch.Tensor, noise_pred_uncond: torch.Tensor, tcfg_rank: int = 1
) -> torch.Tensor:
    batch_size = noise_pred_text.shape[0]
    out_dtype = noise_pred_uncond.dtype

    stacked = torch.stack((noise_pred_text, noise_pred_uncond), dim=1).to(
        dtype=torch.float32
    )
    stacked = stacked.reshape(batch_size, 2, -1)
    _, _, vh = torch.linalg.svd(stacked, full_matrices=False)

    keep_rank = max(1, min(int(tcfg_rank), vh.shape[1]))
    vh_modified = vh.clone()
    vh_modified[:, keep_rank:, :] = 0.0

    noise_pred_uncond_flat = noise_pred_uncond.to(dtype=torch.float32).reshape(
        batch_size, 1, -1
    )
    projected = (noise_pred_uncond_flat @ vh.transpose(-2, -1)) @ vh_modified

    return projected.reshape_as(noise_pred_uncond).to(dtype=out_dtype)


class Flux2KleinTCFGPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    r"""
    The Flux2 Klein pipeline with TCFG for text-to-image generation.
    ...
    def __call__(
        ...
        Args:
            ...
            use_tcfg (`bool`, *optional*, defaults to `True`):
                Enables TCFG SVD-based projection of the unconditional branch before CFG composition.
            tcfg_rank (`int`, *optional*, defaults to `1`):
                Number of right-singular directions kept in TCFG projection. `1` matches the paper setting.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    if use_tcfg:
                        neg_noise_pred = _tcfg_project_uncond_with_svd(
                            noise_pred_text=noise_pred,
                            noise_pred_uncond=neg_noise_pred,
                            tcfg_rank=tcfg_rank,
                        )
