...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from pipeline_stable_diffusion_3_apg import StableDiffusion3APGPipeline

        >>> pipe = StableDiffusion3APGPipeline.from_pretrained(
        ...     "stabilityai/stable-diffusion-3-medium-diffusers", torch_dtype=torch.float16
        ... )
        >>> pipe.to("cuda")
        >>> prompt = "A cat holding a sign that says hello world"
        >>> image = pipe(prompt, guidance_scale=7.0, apg_eta=0.0, apg_step_radius=15.0, apg_momentum=-0.5).images[0]
        >>> image.save("sd3_apg.png")
        ```
"""
...
def _split_parallel_orthogonal_components(
    update: torch.Tensor, reference: torch.Tensor, eps: float = 1e-8
) -> tuple[torch.Tensor, torch.Tensor]:
    # Decompose `update` into components parallel and orthogonal to `reference`.
    # APG uses this decomposition to suppress the parallel component that causes
    # oversaturation and visual artifacts at high guidance.
    reduce_dims = tuple(range(1, update.ndim))
    reference_norm = torch.linalg.vector_norm(reference, dim=reduce_dims, keepdim=True)
    reference_norm = reference_norm.clamp_min(eps)
    unit_reference = reference / reference_norm

    parallel = (update * unit_reference).sum(
        dim=reduce_dims, keepdim=True
    ) * unit_reference
    orthogonal = update - parallel
    return orthogonal, parallel


def _expand_to_tensor_dims(value: torch.Tensor, reference: torch.Tensor) -> torch.Tensor:
    return value.reshape((1,) * reference.ndim)


class StableDiffusion3APGPipeline(
    DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin
):
    r"""
    Stable Diffusion 3 pipeline with APG (Adaptive Projected Guidance).
    ...
    def __call__(
        ...
        Args:
            ...
            guidance_scale (`float`, *optional*, defaults to 7.0):
                Guidance scale for APG. Guidance is enabled by setting `guidance_scale > 1`.
            apg_eta (`float`, *optional*, defaults to 0.0):
                Weight of APG parallel component. `0.0` keeps only orthogonal component, `1.0` restores full update.
            apg_step_radius (`float`, *optional*, defaults to 15.0):
                Fixed L2 radius used to clamp APG guidance update norm (set `<= 0` to disable clamping).
            apg_momentum (`float`, *optional*, defaults to -0.5):
                Reverse-inertia coefficient for APG guidance update. Negative values apply reverse momentum.
            negative_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts not to guide the image generation. If not defined, one has to pass
                `negative_prompt_embeds` instead. Ignored when not using guidance (i.e., ignored if `guidance_scale` is
                less than `1`).
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a [`~pipelines.stable_diffusion_3_apg.StableDiffusion3PipelineOutput`] instead of
                a plain tuple.
        ...
        Returns:
            [`~pipelines.stable_diffusion_3_apg.StableDiffusion3PipelineOutput`] or `tuple`:
            [`~pipelines.stable_diffusion_3_apg.StableDiffusion3PipelineOutput`] if `return_dict` is True, otherwise a
        ...
        self._apg_step_radius = apg_step_radius
        ...
        # 7. Denoising loop
        # APG keeps the original SD3 scheduler step unchanged and only modifies
        # the guidance prediction fed into `scheduler.step`.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    pred_uncond = latents_f32 - sigma * noise_pred_uncond.to(
                        dtype=torch.float32
                    )
                    ...
                    # (1) Projection third: split normalized update into orthogonal/parallel
                    # components w.r.t. conditional prediction and suppress parallel part.
                    ...
                    # Convert guided x0 back to rectified-flow velocity for the scheduler.
