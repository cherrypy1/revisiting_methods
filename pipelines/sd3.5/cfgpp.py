...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from pipeline_stable_diffusion_3_cfgpp import StableDiffusion3CFGPPPipeline

        >>> pipe = StableDiffusion3CFGPPPipeline.from_pretrained(
        ...     "stabilityai/stable-diffusion-3-medium-diffusers", torch_dtype=torch.float16
        ... )
        >>> pipe.to("cuda")
        >>> # CFG++ uses smaller guidance scales (typically 0.0-2.0) compared to standard CFG (5.0-15.0)
        >>> prompt = "A cat holding a sign that says hello world"
        >>> image = pipe(prompt, guidance_scale=1.5).images[0]
        >>> image.save("sd3_cfgpp.png")
        ```
"""
...
class StableDiffusion3CFGPPPipeline(
    DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin
):
    r"""
    Stable Diffusion 3 Pipeline with CFG++ (Manifold-Constrained Classifier-Free Guidance).
    ...
    # CFG++ uses the interpolation scale `lambda` from the paper. `guidance_scale = 0`
    # corresponds to unconditional sampling, and values in [0, 1] interpolate toward the conditional estimate.
    @property
    ...
    def __call__(
        ...
        Args:
            ...
            guidance_scale (`float`, *optional*, defaults to 1.5):
                Guidance scale for CFG++. Unlike standard CFG which uses scales of 5.0-15.0, CFG++ works best with
                smaller scales in the range [0.0, 2.0]. A scale of 1.5 in CFG++ is roughly equivalent to 7.5 in
                standard CFG. Higher values encourage closer alignment to the text prompt but may reduce diversity.
            negative_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts not to guide the image generation. If not defined, one has to pass
                `negative_prompt_embeds` instead. Ignored when not using guidance (i.e., ignored if `guidance_scale` is
                less than `1`).
        ...
        # 7. Denoising loop
        # CFG++ for SD3 keeps the FlowMatch Euler integration unchanged and only
        # changes the model output that is passed into `scheduler.step`.
        # This preserves scheduler internals (step index, optional stochasticity,
        # dtype/device handling) and avoids manual latent updates.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    # 1) Guided branch (same interpolation as standard CFG).
                    ...
                    if skip_guidance_layers is not None and should_skip_layers:
                        ...
                        # Keep the same skip-layer correction used by SD3, but apply it
                        # to the guided predictor before converting it to CFG++ output.
                        noise_pred_guided = (
                            noise_pred_guided
                    ...
                    # 2) CFG++ conversion for Rectified Flow (SD3):
                    #    FlowMatch uses x_sigma = (1 - sigma) * x0 + sigma * eps and predicts velocity
                    #    v = eps - x0. Therefore:
                    #      x0(v)  = x_sigma - sigma * v
                    #      eps(v) = x_sigma + (1 - sigma) * v
                    #
                    #    CFG++ keeps x0 from the guided branch, but uses eps from the unconditional branch:
                    #      x_next_cfgpp = (1 - sigma_next) * x0_guided + sigma_next * eps_uncond
                    #
                    #    `scheduler.step` expects a single velocity for the Euler update
                    #    x_next = x + (sigma_next - sigma) * v.
                    #    We convert the CFG++ target x_next back to that equivalent velocity.
                    scheduler_timesteps = self.scheduler.timesteps
                    step_index = self.scheduler.index_for_timestep(
                        t.to(device=scheduler_timesteps.device), scheduler_timesteps
                    )
                    sigma = self.scheduler.sigmas[step_index].to(
                        device=latents.device, dtype=torch.float32
                    )
                    sigma_next = self.scheduler.sigmas[step_index + 1].to(
                        device=latents.device, dtype=torch.float32
                    )

                    latents_f32 = latents.to(dtype=torch.float32)
                    v_guided_f32 = noise_pred_guided.to(dtype=torch.float32)
                    v_uncond_f32 = noise_pred_uncond.to(dtype=torch.float32)

                    x0_guided = latents_f32 - sigma * v_guided_f32
                    eps_uncond = latents_f32 + (1.0 - sigma) * v_uncond_f32
                    x_next_cfgpp = (
                        1.0 - sigma_next
                    ) * x0_guided + sigma_next * eps_uncond

                    delta_sigma = sigma_next - sigma
                    noise_pred = ((x_next_cfgpp - latents_f32) / delta_sigma).to(
