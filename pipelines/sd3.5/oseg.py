...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from pipeline_stable_diffusion_3_oseg import StableDiffusion3OSEGPipeline

        >>> pipe = StableDiffusion3OSEGPipeline.from_pretrained(
        ...     "stabilityai/stable-diffusion-3-medium-diffusers", torch_dtype=torch.float16
        ... )
        >>> pipe.to("cuda")
        >>> prompt = "A cat holding a sign that says hello world"
        >>> image = pipe(prompt, guidance_scale=7.0, seg_scale=3.0, oseg_scale=1.0).images[0]
        >>> image.save("sd3_oseg.png")
        ```
"""
...
def _blur_orthogonal_query(query, key, blur_sigma, inf_blur):
    ...
    query_parallel = (query * key_norm).sum(dim=-1, keepdim=True) * key_norm
    query = query - query_parallel

    batch_size, heads, _, head_dim = query.shape
    query = query.permute(0, 1, 3, 2).reshape(
        batch_size, heads * head_dim, height, width
    )
    if not inf_blur:
        kernel_size = math.ceil(6 * blur_sigma) + 1 - math.ceil(6 * blur_sigma) % 2
        query = gaussian_blur_2d(query, kernel_size, blur_sigma)
    else:
        query[:] = query.mean(dim=(-2, -1), keepdim=True)

    return query.reshape(batch_size, heads, head_dim, spatial_tokens).permute(
        0, 1, 3, 2
    )


def _expand_seg_applied_layers(
    seg_applied_layers: Optional[List[str]],
) -> Optional[set[str]]:
...
class SEGOSEGSelfAttnProcessor(JointAttnProcessor2_0):
    """Joint-attention processor that blurs the perturbation branch query for SEG/OSEG."""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.do_cfg:
            ...
            key_uncond, _, key_ptb = key.chunk(3)
            query_uncond = _blur_orthogonal_query(
                query_uncond, key_uncond, self.blur_sigma, self.inf_blur
            )
            query_ptb = _blur_orthogonal_query(
                query_ptb, key_ptb, self.blur_sigma, self.inf_blur
            )
            query = torch.cat((query_uncond, query_org, query_ptb), dim=0)
        else:
            query_org, query_ptb = query.chunk(2)
            _, key_ptb = key.chunk(2)
            query_ptb = _blur_orthogonal_query(
                query_ptb, key_ptb, self.blur_sigma, self.inf_blur
            )
...
class StableDiffusion3OSEGPipeline(
    DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin
):
    r"""
    Stable Diffusion 3 Pipeline with OSEG (Orthogonal Smoothed Energy Guidance).

    OSEG replaces the perturbation branch image queries with their orthogonal components relative to keys and then
    smooths those orthogonal queries:
    - `d_cfg = eps_text - eps_uncond_perturbed`
    - `d_oseg = eps_text - eps_text_perturbed`
    - `eps_oseg = eps_text + (w-1)*d_cfg + (lambda_seg + lambda_oseg)*d_oseg`
    ...
    def __call__(
        ...
        Args:
            ...
            seg_scale (`float`, *optional*, defaults to 3.0):
                SEG coefficient `lambda_seg`.
            oseg_scale (`float`, *optional*, defaults to 1.0):
                Additional OSEG coefficient `lambda_oseg` (added to `lambda_seg` in SEG direction).
            seg_blur_sigma (`float`, *optional*, defaults to 9999999.0):
                Gaussian blur sigma for SEG perturbation branch.
            seg_applied_layers (`List[str]`, *optional*):
                Layer tags where SEG/OSEG perturbation is injected. Supports explicit tags (e.g. `d3`, `s7`) and
                ranges (e.g. `d0..12`, `s0..10`). If `None`, all SD3 attention processor layers are used.
            negative_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts not to guide the image generation. If not defined, one has to pass
                `negative_prompt_embeds` instead. Ignored when not using guidance (i.e., ignored if `guidance_scale` is
                less than `1`).
        ...
        if self.do_seg:
            ...
            replace_processor = SEGOSEGSelfAttnProcessor(
                blur_sigma=seg_blur_sigma, do_cfg=self.do_classifier_free_guidance
            )
            ...
            for name in list(procs.keys()):
                ...
                if block_num <= 12:
                    if f"d{block_num}" in self.seg_applied_layers:
                        procs[name] = replace_processor
                else:
                    if f"s{block_num - 13}" in self.seg_applied_layers:
                        procs[name] = replace_processor
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Guidance variants:
                # - CFG
                # - OSEG with orthogonal-smoothed unconditional/conditional paths
                if self.do_classifier_free_guidance and not self.do_seg:
                    ...
                    seg_direction = noise_pred_text - noise_pred_text_perturb
                    noise_pred = (
                        noise_pred_text
                        + (self.seg_scale + self.oseg_scale) * seg_direction
                    )
                elif self.do_classifier_free_guidance and self.do_seg:
                    noise_pred_uncond_perturb, noise_pred_text, noise_pred_text_perturb = (
                        noise_pred.chunk(3)
                    )
                    seg_direction = noise_pred_text - noise_pred_text_perturb
                    ...
                    noise_pred = (
                        ...
                        * (noise_pred_text - noise_pred_uncond_perturb)
                        + (self.seg_scale + self.oseg_scale) * seg_direction
                    )
