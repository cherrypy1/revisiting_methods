...
class PAGCFGJointAttnProcessor2_0:
    """SD3 joint attention processor with PAG identity-mask perturbation."""
    ...
    def _attention(
        ...
        identity_block_size: Optional[int] = None,
    ):
        ...
        encoder_query = encoder_query.view(batch_size, -1, attn.heads, head_dim).transpose(
            1, 2
        )
        encoder_key = encoder_key.view(batch_size, -1, attn.heads, head_dim).transpose(
            1, 2
        )
        encoder_value = encoder_value.view(batch_size, -1, attn.heads, head_dim).transpose(
            1, 2
        )
        ...
        if identity_block_size is not None:
            seq_len = query.shape[2]
            pag_mask = torch.zeros(
                (seq_len, seq_len), device=query.device, dtype=query.dtype
            )
            pag_mask[:identity_block_size, :identity_block_size] = float("-inf")
            pag_mask[:identity_block_size, :identity_block_size].fill_diagonal_(0)
            pag_mask = pag_mask.unsqueeze(0).unsqueeze(0)
            attention_mask = pag_mask if attention_mask is None else attention_mask + pag_mask
    ...
    def __call__(
        ...
        **kwargs,
        ...
        identity_block_size = hidden_seq_len

        if self.do_cfg:
            hidden_uncond, hidden_org, hidden_ptb = hidden_states.chunk(3)
            encoder_uncond, encoder_org, encoder_ptb = encoder_hidden_states.chunk(3)
            hidden_org = torch.cat([hidden_uncond, hidden_org], dim=0)
            encoder_org = torch.cat([encoder_uncond, encoder_org], dim=0)
        else:
            hidden_org, hidden_ptb = hidden_states.chunk(2)
            encoder_org, encoder_ptb = encoder_hidden_states.chunk(2)

        hidden_org = self._attention(
            attn,
            hidden_org,
            encoder_org,
            attention_mask=attention_mask,
        )
        hidden_org, encoder_org = (
            hidden_org[:, :hidden_seq_len],
            hidden_org[:, hidden_seq_len:],
        )
        hidden_org = attn.to_out[0](hidden_org)
        hidden_org = attn.to_out[1](hidden_org)
        if not attn.context_pre_only:
            encoder_org = attn.to_add_out(encoder_org)

        hidden_ptb = self._attention(
            attn,
            hidden_ptb,
            encoder_ptb,
            attention_mask=attention_mask,
        ...
        hidden_ptb, encoder_ptb = (
            hidden_ptb[:, :hidden_seq_len],
            hidden_ptb[:, hidden_seq_len:],
        )
        hidden_ptb = attn.to_out[0](hidden_ptb)
        hidden_ptb = attn.to_out[1](hidden_ptb)
        if not attn.context_pre_only:
            encoder_ptb = attn.to_add_out(encoder_ptb)

        if input_ndim == 4:
            hidden_org = hidden_org.transpose(-1, -2).reshape(
                hidden_org.shape[0], channel, height, width
            )
            hidden_ptb = hidden_ptb.transpose(-1, -2).reshape(
                hidden_ptb.shape[0], channel, height, width
            )
        if context_input_ndim == 4:
            encoder_org = encoder_org.transpose(-1, -2).reshape(
                encoder_org.shape[0], channel, height, width
            )
            encoder_ptb = encoder_ptb.transpose(-1, -2).reshape(
                encoder_ptb.shape[0], channel, height, width
            )

        hidden_states = torch.cat([hidden_org, hidden_ptb], dim=0)
        encoder_hidden_states = torch.cat([encoder_org, encoder_ptb], dim=0)
...
class StableDiffusion3PagPipeline(
    ...
    def __call__(
        ...
        if pag_applied_layers is None:
            pag_applied_layers = ["transformer_blocks.14"]
        self._pag_applied_layers = pag_applied_layers
        self.do_pag = (self._pag_scale > 0.0) and (len(self._pag_applied_layers) > 0)
        ...
        # PAG only
        elif not self.do_classifier_free_guidance and self.do_pag:
        ...
        # change attention layer in Transformer if using PAG

        if self.do_pag:
            orig = self.transformer.attn_processors.copy()
            replace_processor = PAGCFGJointAttnProcessor2_0(
                do_cfg=self.do_classifier_free_guidance
            )
            procs = self.transformer.attn_processors.copy()
            targets = set(self._pag_applied_layers)
            pag_processor_names = []
            for name in list(procs.keys()):
                block_prefix = ".".join(name.split(".")[:2])  # "transformer_blocks.14"
                if block_prefix in targets:
                    procs[name] = replace_processor
                    pag_processor_names.append(name)

            if len(pag_processor_names) == 0:
                raise ValueError(
