...
def _expand_sag_applied_layers(sag_applied_layers: List[str]) -> set[str]:
...
class SAGCFGSelfAttnProcessor(JointAttnProcessor2_0):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # For SAG we should replace scaled_dot_product_attention with hand-written implementation so we could get attention_probs
        # Reference impl: https://github.com/pytorch/pytorch/issues/119188 (removed causal, masking)
        scale = head_dim**-0.5  # head_dim == query.size(-1)
        ...
        self.attention_probs = attention_probs_img.flatten(0, 1).detach()
...
class StableDiffusion3SagPipeline(
    ...
    def __call__(
        ...
        sigmas: Optional[List[float]] = None,
        guidance_scale: float = 7.0,  # authors suggest we should use smaller guidance_scale so effect becomes more clear
        sag_scale: float = 0.75,  # in [0.5; 1.0]
        sag_applied_layers: Optional[List[str]] = None,  # ['d0..12', 's0..10']
        negative_prompt: Optional[Union[str, List[str]]] = None,
        negative_prompt_2: Optional[Union[str, List[str]]] = None,
        negative_prompt_3: Optional[Union[str, List[str]]] = None,
        ...
        if sag_applied_layers is None:
            sag_applied_layers = ["d8"]
        sag_applied_layers = _expand_sag_applied_layers(sag_applied_layers)
        ...
        # 7.1 Setup SAG Processor
        if self.do_sag:
            orig = self.transformer.attn_processors.copy()
            replace_processor = SAGCFGSelfAttnProcessor(
            ...
            for name in list(procs.keys()):
                ...
                if block_num <= 12:  # dual attention for 3.5
                    if f"d{block_num}" in sag_applied_layers:
                        procs[name] = replace_processor
                        sag_processor_names.append(name)
                else:
                    if f"s{block_num - 13}" in sag_applied_layers:
                        procs[name] = replace_processor
                        sag_processor_names.append(name)
            self.transformer.set_attn_processor(procs)
            if len(sag_processor_names) == 0:
                raise ValueError(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # perform self-attention guidance
                # adapted from https://github.com/huggingface/diffusers/blob/main/src/diffusers/pipelines/stable_diffusion_sag/pipeline_stable_diffusion_sag.py
                if self.do_sag:
                    map_size = (
                        latents.shape[-2] // self.patch_size,
                        latents.shape[-1] // self.patch_size,
                    )

                    attention_probs = None
                    for name in sag_processor_names:
                        processor = self.transformer.attn_processors[name]
                        if hasattr(processor, "attention_probs"):
                    ...
                    if self.do_classifier_free_guidance:
                        ...
                        pred_eps = self.pred_epsilon(latents, noise_pred_uncond, t)

                        uncond_attn, cond_attn = attention_probs.chunk(2)
                        degraded_latents = self.sag_masking(
                            pred_x0, uncond_attn, map_size, t, pred_eps
                        )

                        uncond_emb, _ = prompt_embeds.chunk(2)
                        uncond_pooled, _ = pooled_prompt_embeds.chunk(2)

                        degraded_timestep = t.expand(degraded_latents.shape[0])

                        degraded_pred = self.transformer(
                            hidden_states=degraded_latents,
                            timestep=degraded_timestep,
                        ...
                        pred_eps = self.pred_epsilon(latents, noise_pred, t)

                        cond_attn = attention_probs
                        degraded_latents = self.sag_masking(
                            pred_x0, cond_attn, map_size, t, pred_eps
                        )
        ...
        # Change the attention layers back to original ones after SAG was applied
        if self.do_sag:
            self.transformer.set_attn_processor(orig)
    ...
    def sag_masking(self, original_latents, attn_map, map_size, t, eps):
        ...
        attn_map = attn_map.reshape(b, num_heads, hw1, hw2)
        ...
        sigma = self.get_sigma(t, original_latents)
        degraded_latents = (1 - sigma) * degraded_latents + sigma * eps

        return degraded_latents

    def get_sigma(self, timestep, reference):
        if (
            hasattr(self.scheduler, "sigmas")
            and hasattr(self.scheduler, "timesteps")
            and hasattr(self.scheduler, "index_for_timestep")
        ):
            scheduler_timesteps = self.scheduler.timesteps
            step_index = self.scheduler.index_for_timestep(
                timestep.to(device=scheduler_timesteps.device), scheduler_timesteps
            )
            sigma = self.scheduler.sigmas[step_index]
        else:
            sigma = timestep / self.scheduler.config.num_train_timesteps

        sigma = sigma.to(device=reference.device, dtype=reference.dtype)
        if sigma.ndim > 0:
            sigma = sigma.view(-1, *([1] * (reference.ndim - 1)))
        return sigma

    def pred_x0(self, sample, model_output, timestep):
        sigma = self.get_sigma(timestep, sample)
        # x_0 = x_t - sigma * v
        return sample - sigma * model_output

    def pred_epsilon(self, sample, model_output, timestep):
        sigma = self.get_sigma(timestep, sample)
        # epsilon = x_t + (1 - sigma) * v
