...
# SEG AttnProcessor override
# see https://github.com/SusungHong/SEG-SDXL/blob/master/pipeline_seg.py#L71
        ...
        if self.do_cfg:
            ...
            if can_reshape_square:
                ...
                if not self.inf_blur:
                    kernel_size = (
                        math.ceil(6 * self.blur_sigma)
                        + 1
                        - math.ceil(6 * self.blur_sigma) % 2
                    )
                    query_ptb = gaussian_blur_2d(
                        query_ptb, kernel_size, self.blur_sigma
                    )
                else:
...
class StableDiffusion3SegPipeline(
    ...
    # https://github.com/SusungHong/SEG-SDXL/blob/master/pipeline_seg.py#L975
    @property
    def seg_scale(self):
    ...
    def __call__(
        ...
        # https://github.com/SusungHong/SEG-SDXL/blob/master/pipeline_seg.py#L1003
        seg_scale: float = 3.0,
        ...
        Args:
            ...
            seg_scale (`float`, *optional*, defaults to 3.0):
                SEG scale `lambda_seg` controlling strength of the smoothed energy correction.
            ...
            seg_applied_layers (`List[str]`, *optional*):
                ...
                (e.g. `d0..12`, `s0..10`). If `None`, SEG is applied to all SD3 attention processor layers.
            negative_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts not to guide the image generation. If not defined, one has to pass
                `negative_prompt_embeds` instead. Ignored when not using guidance (i.e., ignored if `guidance_scale` is
                less than `1`).
        ...
        # https://github.com/SusungHong/SEG-SDXL/blob/master/pipeline_seg.py#L1233
        ...
        # https://github.com/SusungHong/SEG-SDXL/blob/master/pipeline_seg.py#L1317
        # CFG only
        ...
        # CFG & SEG
            ...
            prompt_embeds = torch.cat(
                [negative_prompt_embeds, prompt_embeds, negative_prompt_embeds],
                dim=0,
            )
            pooled_prompt_embeds = torch.cat(
                [
        ...
        # change attention layer in Transformer if using SEG
        ...
        if self.do_seg:
            ...
            for name in list(procs.keys()):
                ...
                if block_num <= 12:  # dual attention for SD3.5
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # https://github.com/SusungHong/SEG-SDXL/blob/master/pipeline_seg.py#L1397
                # CFG only
                if self.do_classifier_free_guidance and not self.do_seg:
                ...
                # CFG & SEG guidance
                    ...
                    noise_pred_uncond, noise_pred_text, noise_pred_uncond_perturb = (
                        noise_pred.chunk(3)
                    )
                    signal_scale = self.seg_scale
                    noise_pred = (
                        ...
                        + signal_scale * (noise_pred_uncond - noise_pred_uncond_perturb)
                    )
